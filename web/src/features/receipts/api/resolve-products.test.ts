...
describe('resolveProducts — match by code', () => {
  ...
  it('does not overwrite known product metadata with an AI suggestion', () => {
    const result = resolveProducts({
      store: STORE,
      ...
      items: [
        {
          ...
          category: 'Овочі/фрукти',
          product_family_id: 'tomatoes',
          product_variant_id: 'tomatoes_cocktail',
          brand: 'Other',
          is_organic: true,
        },
      ],
  ...
  it('lets a deliberate form override correct or clear existing metadata', () => {
    const result = resolveProducts({
      store: STORE,
      existingProducts: existing([
        {
