...
} from './edited-item-classification';
...
describe('editedItemClassification', () => {
  ...
  it('identifies whether product metadata can be retained during edit', () => {
    expect(isEditedItemIdentityUnchanged([apple], identity, true)).toBe(true);
    expect(
      isEditedItemIdentityUnchanged([apple], { ...identity, product_name: 'Renamed' }, true),
    ).toBe(false);
  });

  it('uses a deliberate form override, including a deliberate clear', () => {
    expect(
      resolvedEditedItemClassification(
        [apple],
        { ...identity, product_family_id: 'tomato', product_variant_id: null },
        true,
      ),
    ).toEqual({ product_family_id: 'apple', product_variant_id: null });
    expect(
      resolvedEditedItemClassification(
        [apple],
        {
          ...
          product_variant_id: null,
          product_metadata_override: true,
        },
        true,
      ),
    ).toEqual({ product_family_id: null, product_variant_id: null });
  });
});
