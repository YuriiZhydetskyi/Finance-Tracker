...
> **Статус (2026-09-22):** усі кроки (1–5, 6.1–6.9) виконано на гілці
> `refactor/2026-09`, окремими комітами замість окремих PR. Відхилення від
> плану: 6.3 — виклик на mount у `StatsPeriodPicker` залишено (через
> `useEffectEvent`), бо на нього спираються тести picker; 1.5 — checkpoint
> замінено тимчасовим типізованим кастом `TODO(types)` у `receipt-bundle.ts`
> до `db push` і регенерації типів; Крок 3 додатково виділив `constants.ts`,
> `job-context.ts`, `job-failure.ts`. Що лишилось до deploy —
> [project-status.md](project-status.md), «Перед merge / deploy».
