...
> **Статус (2026-09-23): завершено й задеплоєно.** Усі кроки (1–5, 6.1–6.9)
> виконано на гілці `refactor/2026-09`, окремими комітами замість окремих PR,
> змерджено в `main` (PR #77). Обидві міграції застосовано до linked Supabase,
> обидві Edge Functions і фронтенд задеплоєно, `database.types.ts`
> регенеровано, тимчасовий каст `TODO(types)` прибрано. Перший деплой падав
> на `supabase link` (CI-токен не мав права читати розкриті API-ключі);
> виправлено окремим PR #80 — `db push` тепер підключається напряму через
> session pooler, деталі в [project-status.md](project-status.md),
> «Deploy (2026-09-23)». Відхилення від плану: 6.3 — виклик на mount у
> `StatsPeriodPicker` залишено (через `useEffectEvent`), бо на нього
> спираються тести picker; Крок 3 додатково виділив `constants.ts`,
> `job-context.ts`, `job-failure.ts`.
