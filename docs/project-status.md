...
регенерація типів; після неї прибрати локальний тип і каст.
...
повторно використовуються за точною ідентичністю; фото зі сторінок додаються
окремо від даних картки. Черга «Без картки» не стверджує, що всі товари мають фото.
Контракт, ліміти, повтор після помилки та обмеження дат —
[ADR-0028](decisions/0028-packaging-pdf-evidence-import.md).
Нових міграцій немає. Ці зміни ще не задеплоєні.

**Пакований товар (2026-09-12):** новий store-agnostic каталог реальних товарів
із харчовою цінністю, штрихкодом, складом і фото упаковки. Міграції
`20260912072149_packaged_products.sql`, `20260912072150_packaging_photos_bucket.sql`
та `20260912072151_packaging_queue_functions.sql` застосовано до linked Supabase.
Рішення — [ADR-0027](decisions/0027-packaged-product-catalogue.md).

Мапінг «як цей товар називається в чеку Aldi/Rewe/…» реалізовано як nullable FK
`products.packaged_product_id`, а не окремою junction-таблицею: `products` уже
задає одну надруковану ідентичність на магазин своїми партіальними унікальними
індексами. Наслідок, заради якого це й обрано: тригер `apply_product_match_rule()`
уже проставляє `items.product_id` на вставці, тож щойно магазинний рядок
привʼязано, кожен майбутній чек цього магазину успадковує картку без коду.

Штрихкод nullable і не є ключем — наявні товари дозаповнюються поступово.
Контрольна цифра GTIN лишається дорадчою (вагові внутрішньомагазинні коди
легітимно не проходять mod-10). Значення харчової цінності ніколи не
перераховуються з колонки «на порцію»: `nutrition_basis` фіксує, з якої саме
надрукованої колонки їх узято.

Початкове внесення даних — вставка JSON від зовнішнього AI на `/packaged-products`. Контракт
`PackagedProductImportSchema` навмисно спільний із майбутньою Edge Function
(`import_source = 'edge-function'`), тож автопарсинг фото не потребуватиме нової
схеми. Черга «Сфотографувати» ранжує позиції без картки за частотою купівлі й
групує той самий товар із різних магазинів в одну задачу;
`products.packaging_not_applicable` прибирає з неї вагові товари та послуги.
`node scripts/test-packaged-products-sql.mjs` перевіряє constraints, RLS,
ранжування черги та розлінкування при видаленні картки на PGlite.

Аналітика БЖВ свідомо поза цією ітерацією: вона потребує окремого рішення про
snapshot нутрієнтів на рядок покупки.

**Таксономія товарів (2026-09-08):** schema migration
`20260907202419_multilingual_product_taxonomy.sql` відновлено локально з точним
версійним номером, який уже є в історії linked Supabase. Вона додає багатомовні
family/variant довідники, catalog attributes `brand`/`is_organic`, purchase
...
чека. Категорії й магазини вибираються чекбоксами з діями «Вибрати всі» / «Зняти
всі». Усі графіки, знижки й списання використовують спільні фільтри. Період,
власні дати та вибір зберігаються в `localStorage` поточного браузера й
відновлюються до першого запиту. «Вибрати всі» включає також майбутні нові
категорії/магазини; порожній вибір означає нічого не враховувати. Якщо браузер
блокує збереження, вибір працює протягом поточного відкриття сторінки.
Поточні запити використовують RPC `stats_*` з міграції
`20260907132451_stats_period_filters.sql`; згадки нижче й у старих архітектурних
схемах про запити до `v_stats_*` описують попередню реалізацію. Ця локальна
доробка не підтверджує стан production.

Деталізація статистики: група (наприклад, «Продукти») → категорія
(«Овочі/фрукти») → розподіл за сімействами («Помідори», «Огірки»). Групу можна
відкрити з таблиці, категорію — з таблиці або натисканням на стовпчик графіка.
Вибраний рівень зберігається в URL (`group`, `category`); період і фільтри
зберігаються під час переходів та повернення назад. Деталі показують підсумок,
динаміку по місяцях, усі магазини та сімейства із сумами й частками витрат.
Частки рахуються від загальної суми поточного рівня у EUR із поверненнями;
за нульового або від’ємного підсумку відсотки не показуються. Позиції без
сімейства включені як «Без визначеного сімейства».
