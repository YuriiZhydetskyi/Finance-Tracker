...
| `web/src/features/products/*`                   | `useProducts` і `useProductTaxonomy` — read-only каталог та family/variant довідники для form/review.                                                                                                                                                                                                                                                               |
| `web/src/features/waste/api/use-waste-items.ts` | Викликає RLS-захищений `search_waste_items`, щоб шукати purchase rows за текстом, family/variant, перекладами, aliases і брендом.                                                                                                                                                                                                                                   |
...
| `web/src/shared/lib/photo-storage/`             | `IPhotoStorage`; `supabase-photo-storage.ts` експортує фабрику `createSupabasePhotoStorage({ bucket })` і два синглтони: `photoStorage` (bucket `receipts`, path `{email}/{yyyy}/{mm}/{ulid}.{ext}`) та `packagingPhotoStorage` (bucket `packaging`, path `{email}/{packaged_product_id}/{ulid}.{ext}`). Bucket параметризовано у фабриці, а не в інтерфейсі порту. |
...
| `packages/domain/src/product-taxonomy.ts` | Zod contracts для family/variant, стабільних taxonomy IDs та інваріанта «variant вимагає family».                                                                                                                 |
...
| `supabase/migrations/20260507000002_storage_bucket.sql`                | `receipts` bucket + 4 RLS policies на `storage.objects`. Bucket `packaging` (фото упаковок) — у `20260912072150_packaging_photos_bucket.sql`.                                                                     |
| `supabase/migrations/20260507000003_stats_views.sql`                   | 4 view-и `v_stats_by_*` з `security_invoker = on`.                                                                                                                                                                |
| `supabase/migrations/20260907202419_multilingual_product_taxonomy.sql` | Family/variant schema, багатомовні category labels/aliases, item snapshot і `search_waste_items`.                                                                                                                 |
...
| `supabase/functions/parse-receipt/prompts/receipt-prompt.ts`           | `buildPrompt(ctx)` + `buildSchema(ctx)` — спільні для обох провайдерів receipt rules і taxonomy IDs; двозначність лишається unknown/manual.                                                                       |
| `supabase/functions/parse-receipt/taxonomy.ts`                         | Звіряє provider suggestion з поточними family/variant довідниками перед persisted шляхом.                                                                                                                         |
...
[4. Edge Function handler.ts]       supabase/functions/parse-receipt/
   ...
   │ loadTaxonomy(auth) → family/variant IDs для provider schema; помилка lookup fail-open до empty taxonomy
   ...
   │ sanitizeParsedReceiptTaxonomy(parsed) → приймає лише існуючий family і його variant
...
| Photo storage        | `IPhotoStorage` (4 methods)             | `supabase-photo-storage.ts` (2 інстанси)   | Cloudflare R2, S3, Backblaze                                         |
