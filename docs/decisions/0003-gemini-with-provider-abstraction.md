# ADR-0003: Gemini 2.5 Flash для парсингу чеків + тонка AiClient-абстракція
...
1. **Google Gemini 2.5 Flash** через AI Studio API + тонкий switch (AiClient.js) з заглушками для OpenAI/Anthropic.
...
Обрано **Gemini 2.5 Flash** з тонкою провайдер-абстракцією.
...
Кожен провайдер — окремий файл (`Gemini.js`, `OpenAi.js`, `Anthropic.js`) з ідентичною сигнатурою `parseReceipt(imageBytes: Uint8Array, ctx: ParseContext) → Promise<ParsedReceipt>`. Спільні типи (`ParsedReceipt`, `ParsedItem`) — у `Domain.js`.
...
- API: `generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent`.
- Auth: API key з AI Studio, зберігається у `PropertiesService.getScriptProperties()` (не в коді).
- Temperature: `0.1` (детермінізм важливіший за креатив).
- Response MIME: `application/json` зі строгою schema.

## Pros and Cons of the Options

### 1. Gemini 2.5 Flash + AiClient switch
