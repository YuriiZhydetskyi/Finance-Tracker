...
### FX fallback rule
- ECB не публікує курси у вихідні і свята. NBU (для UAH) — те саме.
- Якщо курсу на дату чеку немає — беремо **останній доступний курс ≤ дата чеку**.
- Курс на чеку фіксується назавжди — навіть якщо джерело пізніше перегляне архів.

### Джерела курсів
- **ECB Reference Rates** — основне джерело. Покриває ~30 мажорних валют (USD, GBP, JPY, PLN, CZK, ...). **UAH немає в цьому списку.**
- **NBU (bank.gov.ua)** — джерело для UAH. Один запит = одна дата; backfill через `UrlFetchApp.fetchAll` паралельно.
- При додаванні нової валюти — спочатку перевір, чи є вона в ECB feed. Якщо ні — додай fetcher по аналогії з NBU.
